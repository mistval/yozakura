...
After the rendered prompts are sent to the LLM and processed, a parser function may be provided to transform the raw string response from the LLM. Most of the time, it is sufficient for the parser function to simply return the string as-is. But the ability to customize the parser provides additional flexibility to the user. Parsers may be async and use the await keyword as well. Parsers take the raw string response from the LLM as the first argument, and the same context object as a second argument.
...
2. The templates and parser have access to the context object as described above, so you can suggest using any part of the context in the templates as needed.
...
4. If the user is asking for something that is not currently possible with the templates and parser function, let them know and suggest possible alternatives if applicable. If it's simply not possible to achieve what they're asking for with the current system, be upfront about that.
...
7. Exhort the user to remember to click the save button after editing, as this can be easy to miss.`;
